...
package com.beauscott.mobile.spark.components.supportClasses
{
...
//            currentViewMenu.height = getLayoutBoundsHeight();
    ...
    protected function updateViewMenuPosition():void {
        currentViewMenu.width = getViewMenuPreferredWidth();
        currentViewMenu.height = getViewMenuPreferredHeight();
        currentViewMenu.move(0,0);
    }
    
    protected function getCurrentViewMenu():ViewMenu {
        return currentViewMenu;
    }
    
    protected function getViewMenuPreferredWidth():Number {
        return getLayoutBoundsWidth();
    }
    
    protected function getViewMenuPreferredHeight():Number {
        return getLayoutBoundsHeight();
    }
        
    /**
     *  @private
     */ 
    protected function openViewMenu():void
    {                
        if (!currentViewMenu)
        {
            currentViewMenu = ViewMenu(viewMenu.newInstance());
            
            // AB MONKEYPATCH (probably a better, skinned way of doing this...
            currentViewMenu.resizeForSoftKeyboard = this.resizeForSoftKeyboard;
            // END AB MONKEYPATCH
    ...
    private function viewMenuClose_handler(event:PopUpEvent):void
    {
        ...
        // Restore focus
        if (!(systemManager.stage.focus is DisplayObject) || contains(systemManager.stage.focus as DisplayObject))
